<form action="{{ route('admin.job-openings.update', $jobOpening) }}" method="POST" x-data="{
    title: '{{ old('title', $jobOpening->title) }}',
    slug: '{{ old('slug', $jobOpening->slug) }}',
    slugEdited: true,
    generateSlug(value) {
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            ...
            <x-common.component-card title="Job Details">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" x-model="title"
                        @input="generateSlug($event.target.value)" value="{{ old('title', $jobOpening->title) }}"
                ...
                <div>
                    ...
                    <input type="text" id="short_description" name="short_description"
                        value="{{ old('short_description', $jobOpening->short_description) }}"
                ...
                <div>
                    ...
                    <x-form.quill-editor name="description" :value="old('description', $jobOpening->description)"
        ...
        <div class="space-y-6">
                        ...
                        <select id="work_type" name="work_type"
                            ...
                            @foreach (['remote' => 'Remote', 'wfa' => 'WFA', 'wfo' => 'WFO', 'hybrid' => 'Hybrid'] as $value => $label)
                                <option value="{{ $value }}"
                                    ...
                                    {{ old('work_type', $jobOpening->work_type) === $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative">
                        <select id="status" name="status"
                            ...
                            <option value="open" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                {{ old('status', $jobOpening->status) === 'open' ? 'selected' : '' }}>Open</option>
                            <option value="closed" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                {{ old('status', $jobOpening->status) === 'closed' ? 'selected' : '' }}>Closed</option>
                        </select>
                        <span
                            class="pointer-events-none absolute top-1/2 right-4 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.job-openings.index', $jobOpening) }}"
